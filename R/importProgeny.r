...
#'
#' @description \code{readInData()} imports data exported from a Progeny database.
#'
#' @param inputFile the .txt file containing the biological and genotype data to be imported into \pkg{idfgen}. The data exported from
#' Progeny should be in the following format. Export Format: Wide (One Row per Individual); Identification: Unique ID = Individual Name & Export
#' Using Pedigree and ID; Allele Format: Two Columns Per Call & Export No Calls as "0"; SNP Coding: A,C,G,T; Allele Call: 142,144,146;
#' Pedigree Options: Use pedigree names. Any additional fields may be added.
...
importProgeny <- function(inputFile, genotypeStartColumn, popColumn = 1, sortBPs = FALSE)
{
#
...
# read file
  rawData <- read.table(inputFile, quote="", header = TRUE, sep="\t", row.names = 2, as.is = TRUE, colClasses = "character", comment.char = "")
